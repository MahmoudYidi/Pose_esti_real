...
class GpuNetPub
{
	...
	int32_t *lo = &c;
        ...
        std::vector<uint8_t> img_msg_data(360*640*3);
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//the same format as MATLAB uses.
        ...
        int32_t sub_height = 360;
        int32_t sub_width = 640;
        ...
        //Call generated network on the input data
       	...
       	yolov2_detect(input,output, &a, &b); //bbox);
...
//so as to be able to publish as image data
  std::vector<uint8_t> img_msg_data1(224*224*3);
	int32_t sub_height1 = 224;
        int32_t sub_width1 = 224;

      int32_t o_idx = 0;
      int32_t m_idx;
      int32_t i_idx;
          
        for(int32_t i = 0; i<(sub_height1); i++)
        {
            for(int32_t j = 0; j<(sub_width1); j++)
            {
                for(int32_t k = 0; k<(3); k++)
                {
                    i_idx = (sub_width1*sub_height1*k) + (sub_height1*j) + (i);
                    msg_idx = 3*width*(floor((height - sub_height1)/2) + i) + 3*(floor((width - sub_width1)/2) + j) + (k); // If the image is 227x227x3 : (width*3*i) + (3*j) + (k);
                     img_msg_data1[o_idx] = output[i_idx];
...
//      std::vector<uint8_t> img_msg_data(std::begin(output), std::end(output));
  ...
  //dataoutmsg.data = bbox_size;
    //    data_pub.publish(dataoutmsg);
        ...
        dataoutmsg.y = b;
        data_pub.publish(dataoutmsg);
        imgoutmsg.height = sub_height1;
        imgoutmsg.width = sub_width1;
        imgoutmsg.encoding = "rgb8";
        imgoutmsg.step = 3*sub_height1;
        
        
	imgoutmsg.data = img_msg_data1;
